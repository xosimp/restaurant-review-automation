...
| `dsr_sweep` | every tick, claimed per 10-minute slot | `dsr.pipeline.run_sweep` — the nightly DSR for every live, `dsr_enabled`, POS-connected restaurant past its own *local* close: polls the POS close-day record every 10 min, collects each block (retrying awaiting ones 10/20/40 min), writes the narrative, finalises; provisional at `dsr_deadline_hour` *local* (default 4am), a new version when late data lands (checked hourly for 48h). Bounded (20 min) with a `job_cursors` cursor; each night claims `dsr:<rid>:<date>:v<n>` so it never double-runs |
