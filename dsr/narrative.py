...
def settle_actions(actions, F, ctx, declined, dropped):
    ...
    try:
        import rec_ledger
        # owner_only: an action resting on a figure the Manager DSR never
        # shows (budget, prime cost, loss lines, the Food block) is never
        # listed to a manager in the owner's recommendation record either.
        batch = [{"key": a["key"], "module": _MODULE[action_block(a)], "kind": "dsr_action",
                  ...
                  "model_written": True, "owner_only": any(owner_only_cite(c) for c in a["cites"])}
