...
### The nightly report's shared sections (9/25/26)

Order, both reports: the summary (owner "Executive summary", manager "Operations summary") → the owner's Today's score with wins and risks, or the manager's **Today's shift** (a grid of big numbers, amber when a no-show, late arrival or overtime happened) and **Operations** → **Top KPIs** → **AI insights** → **Tomorrow's priorities** → **Tomorrow** → the blocks (sales, labor, food, reviews, weather, then the rest) → **How did yesterday turn out?**. A **KPI tile** (`.dr-kpi` / `DSRKPIGrid`) is the value in the number face with a 14-night trend line (ink stroke, the latest point ember), then its direction — "↓ 1.4 pts vs last Saturday" in good/bad tone by the metric's better side — a streak ("Best Saturday in 5 weeks"), the target and, only when fair, "Restaurants like yours" (otherwise "no fair comparison yet" in muted text). **Tomorrow** lists prep items with an amber triangle (a plain dot when informational), the AI recommendation, Cavnar's forecast and the **AI confidence** as a percentage with "Based on …" — the forecast's measured record (`confidence.track`), and "—" with that line when there is no record yet (`pct` null); the weather, schedule and events it lists are `watch`, things to keep an eye on, never "confidence". **How did yesterday turn out?** marks each prediction ✓ correct (good) / ✗ incorrect (bad) / • not graded, with the measured figure, and the prediction accuracy as a percentage once five are graded — the count before that.
Order, both reports: the summary (owner "Executive summary", manager "Operations summary") → the owner's Today's score with wins and risks, or the manager's **Today's shift** (a grid of big numbers, amber when a call-off, late arrival or overtime happened) and **Operations** → **Top KPIs** → **AI insights** → **Tomorrow's priorities** → **Tomorrow** → the blocks (sales, labor, food, reviews, weather, then the rest) → **How did yesterday turn out?**. A **KPI tile** (`.dr-kpi` / `DSRKPIGrid`) is the value in the number face with a 14-night trend line (ink stroke, the latest point ember), then its direction — "↓ 1.4 pts vs last Saturday" in good/bad tone by the metric's better side — a streak ("Best Saturday in 5 weeks"), the target and, only when fair, "Restaurants like yours" (otherwise "no fair comparison yet" in muted text). **Tomorrow** lists prep items with an amber triangle (a plain dot when informational), the AI recommendation, Cavnar's forecast and the **AI confidence** as a percentage with "Based on …" and "Missing: …". **How did yesterday turn out?** marks each prediction ✓ correct (good) / ✗ incorrect (bad) / • not graded, with the measured figure, and the prediction accuracy as a percentage once five are graded — the count before that. Labor over a target the owner never set (Cavnar's starting target, `detail.target_source` "default") is amber, never red — the Labor tile, the email's Labor stat and Today's score agree. An estimate (food cost, prime cost) says so everywhere: `est.` on the tile, "(est.)" in the email. The manager's Top KPIs never repeat a tile Operations already shows.
