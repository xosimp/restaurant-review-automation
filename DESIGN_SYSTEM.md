...
large screen, so every tab's margins line up. The one exception is Labor's schedule workspace (`.sw`, three panels, up to 1480px — see Schedule workspace), which the owner asked for; the rest of Labor stays in the column. Reviews is one column too:
...
| Schedule workspace (web) | `.sw` (`#sched-ws`, owner 9/26/26) — the one Labor block wider than the shared 1120px column: `width:min(1480px, 100vw − 48px)`, centred on it. `.sw-grid` is three columns: `.sw-left` 272px (Generation settings: the week picker and shifts CSV · Forecast: each day's forecast hours as `.sw-bar`s, forecast sales and its basis, holidays ahead · Labor target: the % input saving to `account/targets`, this week's PAR · Employees and Constraints: summaries from `/api/labor/roster` and `/api/labor/rules` that open the Roster / Rules editors in Team & rules · Advanced AI: the Thursday auto-draft switch, `sched_notes` "Notes Cavnar AI follows on every draft", Redo some days · Generate, sticky at the panel's foot), `.sw-center` (idle `.sw-idle` with the reopen line; the building-the-week animation takes the whole column while a draft runs; the draft card with Week \| List `.lb2-range` tabs), `.sw-right` 344px (Shift quality · AI suggestions · Warnings · Coverage · Labor · Cost · Opportunities · Apply fixes — each `.sw-sec` shows only when it has something to say, and only once a week is open). Sides are sticky cards with their own scroll; sections are separated by space (`.sw-k` ember kickers), never rules. Under 1320px the right panel drops below in two columns; under 900px everything stacks. **Week grid** `.swg`: one `.swg-r` row per person (everyone on the roster, grouped under `.swg-role` by their most-worked role), a column per day (`.swg-d`: day, date, people on · hours, the heaviest day's name in ember), each shift a `.swg-chip` (time in the number face, hours and any other role under it; `.bad` a red edge for a shift that breaks a rule, `.chg` a blue dot for one the manager changed). The person's week hours turn red past the weekly ceiling. A chip opens the `.swp` popover (who, day, role, start, end; Remove / Cancel / Done) that runs the list editor's own `schedFinishEdit`; a cell's `+` adds a shift from that person's usual times. Rows patch in place (`_schedPatchChildren`), and a removed shift fades out. **Coverage** also compares each role's scheduled hours with what its roster covers at the weekly ceiling and names the role when it is over |
