...
| AI search presence results (web) | `.in2-aiv-grid`: two recessed `.in2-aiv-card`s side by side (How often AI names you with the orbit; Listing strength with its checklist, one item per row), then "Every check, by question" (each question across every check: a dot per run and its count) and "Latest check: what AI said" (the latest run's answers) as full-width cards (`.in2-aiv-wide`). Each card's header is `.in2-aiv-h`: the label left, a tone pill (`i.good/.warn/.bad`) right that wraps under the label rather than running into the next card; the value is `.in2-aiv-v`, its caption `.in2-aiv-cap` under it. One column under 1000px. The body has no height cap, so the loading radar and late-loading rows never clip |
