...
large screen, so every tab's margins line up. Reviews is one column too:
...
| AI search presence results (web) | `.in2-aiv-grid`: two recessed `.in2-aiv-card`s side by side (How often AI names you with the orbit; Listing strength with its checklist, one item per row), then By question and AI query results as full-width cards (`.in2-aiv-wide`). Each card's header is `.in2-aiv-h`: the label left, a tone pill (`i.good/.warn/.bad`) right that wraps under the label rather than running into the next card; the value is `.in2-aiv-v`, its caption `.in2-aiv-cap` under it. One column under 1000px. The body has no height cap, so the loading radar and late-loading rows never clip |
...
| What changed (Intel) | Under Nearby competitors on both platforms (`#in2-moves` / `IntelView.movementSection`), from `/api/intel/movement`: the kicker "What changed", one quiet line ("New and gone since the check on 9/14/26", or that nothing opened, closed or moved past normal ups and downs, or — before two weekly checks exist — that it needs two to compare), then rows: the name with an ember "New nearby" / "No longer nearby" tag, and up to three rating moves past the noise floor as "4.2 → 4.5★ · +38 reviews" |
