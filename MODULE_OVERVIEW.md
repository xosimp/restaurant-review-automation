...
- **After close** — `closeout.py` is the manager's handoff: four quick lines plus the six fields the nightly DSR reads (equipment, VIP guests, maintenance, shift notes, general notes, and "why the day went how it did"). It leads the next brief (equipment and maintenance included — they are the opener's problem), and `dsr/block_closeout.py` carries it verbatim into the night's report.
