...
**People** (`people.py`, Friction #25): one person record composed from the stores that own each fact — roster settings (`staff_settings`), the staff login (`memberships`), contacts (`staff_contacts`), ratings (capabilities), portal availability and the per-role rate. It stores nothing; `update_person` writes each field back to its owner, after every refusal is checked (a PIN or job title needs the owner; a changed role or pay rate is refused in words — the role comes from the shifts, the rate from Labor's per-role rates; F3-5/F2-13). Two names that slug alike get distinct keys. `people.reach` picks a person's channel and `people.tell` sends one staff notice on it (the app, a consented text, email) — used by publish, time-off decisions and shift-request outcomes (F2-5/F2-12). `/api/people[/<key>]` (web + mobile) backs the web person sheet (`openPersonSheet`, opened from the roster, the team list, Account → People and the send drawer) and iOS. **The open draft** (Friction #4): `labor/publish-check` resolves the newest unsent week; Labor reopens it in the editor on open, history rows and Home's "Send now" reach it, and Send is one "Save & send to N staff" button that saves edits first and names rule warnings in its label. On a week staff already have, a save tells nobody; Send tells the people the saved changes moved (`client_api.send_schedule_changes` — SCHEDULE_PUBLISH, the rule gate and the undo window, like the first send; F2-2).
...
- **The Command Center** — `command_center.py`: the commands a login may run here (`registry`, Ask's write tools through the same `tool_allowed` test), one permission-filtered search, and `propose` (Ask's confirm card from `build_proposal`, logged `proposed` with `surface='command'` so a closed preview never becomes a Still-open item, and its path ids in `target` so a reopen rebuilds the same card; settled in the action's own request through `settle_confirmed` — the `X-Cavnar-Proposal` header — or Ask's own action route). Home's Still open sends (Send now, a request's Approve / Deny) open this card too. The web ⌘K palette and the iOS command sheet read the same routes. Deterministic: free text goes to Ask's stream, so Response Validation and the readiness gate stand in front of every answer. Anything that acts follows the confirm/undo policy (`DESIGN_SYSTEM.md` §10): reversible at once with Undo, outward through the confirm card, `confirm()` only for security.
...
The iOS staff portal (`Features/Staff/`) has the same requests as the web one (Friction audit #49): a **Requests** tab (time off with withdraw, their own drops and swaps, swaps a colleague asked of them, open shifts to pick up) and a menu on each shift — "Can't work this" / "Swap with a colleague" (`StaffRequestsViews.swift`). Since the 9/25/26 parity pass it also has the rest of the web portal (`StaffSelfServiceViews.swift`): **Before service** on the Schedule tab (`/staff/api/preshift`, hidden when empty), **My availability** (the days they can't work + a note) and **What I'd like** (dayparts, hours a week, and the separate "text me when my schedule is posted" opt-in, saved on change) at the top of Requests, and **Change your PIN** on Profile (needs the current PIN; the change ends their staff sessions, so the app signs out). It posts the web portal's `/staff/api/*` routes one for one; there are no mobile twins for the staff tier.
...
**Design stance**: every way in only opens a place. A quick action, a shortcut, a widget or Live Activity tap, a `cavnarai://nav/…` or `dashboard.cavnar.ai` link becomes a nav path posted as `.cavnarOpenNav` — the router owns the navigation, the Face ID lock still stands in front (a destination waits in `SystemEntry` until the scene is active), and nothing outward is sent without the in-app confirm card. The one exception is Undo on a pending send (`delayed.py`), the safe direction, which needs an unlocked device. The widget extension never signs in: the app writes `WidgetSnapshot` into the `group.ai.cavnar.CavnarAI` App Group and clears it on sign-out. The command sheet reads the Command Center's mobile routes (`/mobile/api/command/registry|search|propose`), confirms through Ask's own `ProposalCard` and route, answers time-off and shift requests in place with the Labor decide routes, and falls back to navigation + Ask on a server without those routes. Module screens open the section a path names from their ROUTE — `ModuleRoute` carries the whole `NavPath`, query and all, from every way in (a push, a notification row, a Home card, the command sheet, a quick action): Labor's one handler (`LaborFocus`: `labor/requests`, `request/…`, `labor/schedule`, `person/<key>` opens that person's sheet) and Food Cost (`inventory/invoices?scan=camera`, `inventory/order`, `inventory/count`). The section inbox that raced the route was removed (F3-7). A LINK (`cavnarai://…` or a universal link) opens a place only: an Ask question is filled in, never sent, and `location/<id>` opens the switcher (F3-12). Every location switch goes through `SessionStore.didSwitchLocation`, which calls RootView's reset (both stacks, Home, chrome, widget) before the caller opens anything (F3-4). The widget reads `/actions?peek=1` and `/dsr/<day>?peek=1`, keeps each half on its own clock (a waiting count is current for 6 hours, last night's figures for 36), never carries one location's figures into another's, and is cleared by sign-out itself. `action/<id>` is a queued send (delayed_actions); an Ask proposal opens as `proposal/<id>` (or `action/<id>?kind=proposal`) in `ProposalReopenSheet`. Universal links: the app has `applinks:dashboard.cavnar.ai` and `hosted_dashboard.py` serves `/.well-known/apple-app-site-association` for exactly `/dashboard` (app id `8DW8XL63K6.ai.cavnar.CavnarAI`).
