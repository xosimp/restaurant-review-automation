...
struct GuestTextClubView: View {
    ...
    private func joinLinkCard(_ url: String) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Guest join link").font(.cavnarBody(16, weight: 700)).foregroundStyle(Color.cavnarInk3)
    ...
    private var campaignCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("Send a campaign").font(.cavnarBody(16.5, weight: 700)).foregroundStyle(Color.cavnarInk)
            ...
            (Text("Goes to ")
                + Text("\(viewModel.selectedSegmentCount)").font(.cavnarNumber(16, weight: 700))
    ...
    private var newsletterCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                Text("Email newsletter").font(.cavnarBody(16.5, weight: 700)).foregroundStyle(Color.cavnarInk)
                Spacer()
                (Text("\(viewModel.subscriberCount)").font(.cavnarNumber(16, weight: 700))
                    + Text(" subscribed"))
                    .font(.cavnarBody(16))
                    .foregroundStyle(Color.cavnarInk3)
            }
            ...
            if viewModel.subscriberCount == 0 {
                ...
                if let result = viewModel.newsletterResult {
                    Text(result).font(.cavnarBody(16, weight: 600)).foregroundStyle(Color.cavnarGreen)
                }
    ...
    private var historyCard: some View {
        if !viewModel.campaigns.isEmpty {
            VStack(alignment: .leading, spacing: 10) {
                Text("Campaigns sent").font(.cavnarBody(16.5, weight: 700)).foregroundStyle(Color.cavnarInk)
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        HStack {
                            Text(campaign.whenLabel)
                                ...
                                .foregroundStyle(Color.cavnarEmber)
                            Spacer()
                            (Text("\(campaign.sentCount)").font(.cavnarNumber(16, weight: 700))
                                + Text(" sent")
                                + Text(campaign.clicks > 0 ? " · " : "")
                                + (campaign.clicks > 0
                                   ? Text("\(campaign.clicks)").font(.cavnarNumber(16, weight: 700)) + Text(" taps")
                                   : Text("")))
                                .font(.cavnarBody(16))
                                .foregroundStyle(Color.cavnarInk3)
                        }
    ...
    private var ledgerCard: some View {
        if let ledger = viewModel.ledger {
            VStack(alignment: .leading, spacing: 10) {
                Text("Consent & sending").font(.cavnarBody(16.5, weight: 700)).foregroundStyle(Color.cavnarInk)
                HStack(spacing: 0) {
                ...
                (Text("\(ledger.textsThisMonth)").font(.cavnarNumber(16, weight: 700))
                    + Text(" texts sent this month across ")
                    + Text("\(ledger.campaignsThisMonth)").font(.cavnarNumber(16, weight: 700))
        ...
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                VStack(alignment: .leading, spacing: 2) {
                    (Text("Guest contacts (") + Text("\(viewModel.contacts.count)").font(.cavnarNumber(16.5, weight: 700)) + Text(")"))
                        .font(.cavnarBody(16.5, weight: 700))
                        .foregroundStyle(Color.cavnarInk)
                    if !viewModel.contacts.isEmpty {
                        (Text("\(viewModel.textableCount)").font(.cavnarNumber(16, weight: 700))
                            + Text(" text-eligible"))
                            .font(.cavnarBody(16))
                            .foregroundStyle(Color.cavnarInk3)
                    }
        ...
        HStack(alignment: .top) {
            VStack(alignment: .leading, spacing: 2) {
                Text(contact.name?.isEmpty == false ? contact.name! : "Guest")
                    .font(.cavnarBody(16.5, weight: 600))
                    .foregroundStyle(Color.cavnarInk)
                Text(contact.phone).font(.cavnarNumber(16)).foregroundStyle(Color.cavnarInk3)
