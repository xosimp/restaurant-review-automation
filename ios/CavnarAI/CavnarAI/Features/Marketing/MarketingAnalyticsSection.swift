                ...
                if let perf = viewModel.performance, perf.hasData {
                    if let top = perf.topPost {
                        VStack(alignment: .leading, spacing: 6) {
                            Text("Top post").font(.cavnarBody(16, weight: 700)).foregroundStyle(Color.cavnarInk3)
                            Text(top.topic ?? "").font(.cavnarBody(16.5, weight: 600)).foregroundStyle(Color.cavnarInk)
        ...
        if let window = viewModel.window {
            VStack(alignment: .leading, spacing: 12) {
                Picker("Period", selection: Binding(
                    ...
                    set: { days in Task { await viewModel.setWindow(days) } })) {
                    Text("7 days").tag(7)
                    Text("30 days").tag(30)
                    Text("90 days").tag(90)
                }
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        ForEach(window.byPlatform) { platform in
                            HStack {
                                ...
                                Spacer()
                                (Text("\(platform.posts)").font(.cavnarNumber(16, weight: 700))
                                    + Text(" posts · ")
                                    + Text("\(platform.reach)").font(.cavnarNumber(16, weight: 700))
                                    + Text(" reach · ")
                                    + Text("\(platform.engagementRate)%").font(.cavnarNumber(16, weight: 700)))
                                    .font(.cavnarBody(16))
                                    .foregroundStyle(Color.cavnarInk3)
                            }
                        }
            ...
            ForEach(viewModel.recentTopics) { topic in
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    if let line = topic.metricsLine {
                        Text(line).font(.cavnarNumber(16)).foregroundStyle(Color.cavnarInk3)
