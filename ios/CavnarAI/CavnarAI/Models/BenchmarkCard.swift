...
struct BenchmarkRow: Decodable, Hashable, Identifiable {
    ...
    enum CodingKeys: String, CodingKey {
        case metric, label, kind, standing, tone, behind, against, note, action
    ...
    /// "Labor % 35% — worse than your normal"
    var headline: String {
        [label + (valueText.map { " \($0)" } ?? ""), standing].compactMap { $0 }.joined(separator: " \u{2014} ")
    }

    /// "vs your own previous 13 weeks (30.3%) · as of 9/20/26" — what the
    /// standing is read against, its strength and its date.
    var detail: String? {
        var bits: [String] = []
        if let against { bits.append("vs \(against)" + (middleText.map { " (\($0))" } ?? "")) }
        if let strengthPct { bits.append("\(strengthPct)% comparison strength") }
        if let asOf { bits.append("as of \(asOf)") }
        return bits.isEmpty ? nil : bits.joined(separator: " \u{00B7} ")
    }
}

/// The honest state below the minimum: what the card falls back to and the
/// engine's reason there is no like-for-like group.
struct BenchmarkBelowMinimum: Decodable, Hashable {
    var text: String? = nil
    var whyNot: String? = nil
    enum CodingKeys: String, CodingKey { case text; case whyNot = "why_not" }
}
