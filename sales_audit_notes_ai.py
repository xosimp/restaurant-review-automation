...
- Effects: "lower_confidence" when a note shows a sized opportunity is less real than the numbers imply (a reason the gap exists that the product will not fix, a number the owner guessed, a one-off period). "raise_confidence" when a note corroborates a sized gap with a concrete fact (kept as corroboration beside the figure; a note never raises a band). "context" for everything else worth carrying into the results. Do not attach an effect to a category that has insufficient data — suggest the missing answer instead if the note contains it.
