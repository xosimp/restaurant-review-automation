...
def test_inventory_context_names_critical_and_reorder_items(db_path, monkeypatch):
    ...
    # **kw: analysis_for now also passes the measured inputs analyse_inventory
    # can't derive for itself (real windowed purchases, the actual count dates).
    monkeypatch.setattr(inventory, "analyse_inventory", lambda items, delivery_days=None, upcoming_holidays=None, today=None, **kw: {
