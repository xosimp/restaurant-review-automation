...
def test_web_labor_money_is_a_gap_never_green_and_never_on_sample_data():
    ...
    assert "labor.is_live and savings_breakdown.labor_vs_industry_monthly > 0" in s  # hidden at $0
...
def test_web_dsr_reads_the_renamed_keys_first_and_labels_them_as_opportunities():
    s = _dash()
    assert "['largest_money_opportunity|largest_dollar_gap|largest_money_saving','Largest dollar gap · an opportunity, not savings']" in s
