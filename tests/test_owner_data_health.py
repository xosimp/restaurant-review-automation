...
def test_slots_are_the_schedulers_own_times():
    ...
    # The weekly reads run first on Monday (catching up through Wednesday
    # on the ISO week's claim, DH2-10): the next slot owners are told is
    # Monday's.
    assert dh.SLOTS["competitor"] == ((6,), 0) and \
        '_due(now, 6) and now.weekday() <= 2 and _ops.claim_period("competitor_analysis", _iso_week)' in src
    assert dh.SLOTS["visibility"] == ((7,), 0) and \
        '_due(now, 7) and now.weekday() <= 2 and _ops.claim_period("ai_visibility", _iso_week)' in src
