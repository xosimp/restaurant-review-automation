...
- `restaurants`: `fiscal_week_start_dow`, `fiscal_year_start`, `fiscal_period_scheme` (4x13 | 445), `dsr_enabled`, `dsr_deadline_hour`, `dsr_notify` (0 by default: the report builds and shows in the app; email and push start only when the owner turns this on — and the old closing-summary push stays until then).
