...
- `dsr_predictions` — UNIQUE (restaurant_id, for_date, key): what a night's report predicted about the NEXT night (`dsr.predictions`, "How did yesterday turn out?", 9/25/26). `key` sales_range | sales_budget | rain | event; `metric` the fact it is graded on (sales.net); `op` between | gt | lt with `value` or `low`/`high`; `text` the claim as the owner reads it; `basis` what it rests on; `made_on` the report's night. Written ONCE before its night (INSERT OR IGNORE — a re-run never rewrites a prediction after the fact); `outcome` correct | incorrect | NULL (ungraded — a night with no sales is never "wrong"), `actual`, `graded_at` set from that night's Sales block, re-graded by each version. `accuracy()` reads the last 30 days' graded rows.
