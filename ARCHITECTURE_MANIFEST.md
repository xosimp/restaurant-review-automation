...
| `templates/` | Jinja templates. `dashboard.html` is the entire client web app (markup + inline CSS + inline ES5 JS); the rest are standalone pages (auth, staff portal, admin, status, audits) and three partials (`_csrf_fetch.html`, `_review_card.html`, `_fc_receive_js.html` — receiving deliveries, shared by the full and counts-only Food Cost panels). | web |
...
| `ios/CavnarAI/` | The SwiftUI app. `project.yml` → `./generate.sh` → gitignored `.xcodeproj`. `CavnarAI/Features/<Module>/` mirrors the product modules; `DesignSystem/` is the one place for tokens, type, motion and shared components. `CavnarWidgets/` is the WidgetKit extension (widget + Live Activity; never signs in), `Shared/` the files compiled into both targets. | iOS |
...
| `dsr.store` | 2 | DSR reports (one row per restaurant, business date and version; stage times as provenance), `dsr_metrics` (the searchable per-night history: `metric_series`, `find_days`; exactly the finished version's READY blocks — `sync_metrics`), budgets, the POS-department → DSR-category map (unmapped is shown, never guessed), the ONE resolver for another night's net (`baselines_net`: DSR → imported workbook → POS sync only where it is the DSR's basis) and last year's night (`last_year_day`, fiscal-calendar aware), and the clock-in check's record (`dsr_coverage_runs`) |
...
| `dsr.kpis` | 2 | the night's key numbers WITH direction for both views (9/25/26): the value, the change against the same weekday (points for a %, percent for money and counts), "Best/Worst <weekday> in N weeks", a 14-night trend line, the target (`thresholds.target_for`) and — only when the Benchmark Engine has a fair one — restaurants like yours; derived sales per labor hour, prime cost % ((labor $ + estimated recipe cost) ÷ net, withheld under the recipe-coverage floor — never two percentages with different bases added) and beverage mix (the Liquor/Beer/Wine categories matched whole). `OWNER_SET` / `MANAGER_SET` (operations, not finance; disjoint from `OPERATIONS_SET`) and the manager's `shift_recap` and operations, every figure through `dsr.access.line_allowed` |
| `dsr.tomorrow` | 2 | the next night as a report goes out: prep items (approved/pending time off, the NWS forecast, listed events and reservations, critically low stock, the published schedule), Cavnar's forecast and an AI confidence % that is only the forecast's measured record — how often its stated range held, out of sample (`demand.demand_accuracy`), with no % under 10 ranged nights; weather, schedule and events are things to watch, not confidence. Built once by the pipeline (`facts["tomorrow"]`, read only per view through `access.tomorrow_for`), deterministic |
| `dsr.predictions` | 2 | "How did yesterday turn out?": gradable predictions about the next night (the forecast's range, vs budget, rain, a listed event), written once strictly before the night opens into `dsr_predictions` (a row written later is void), graded from that night's Sales block, and the rolling 30-day accuracy (a % from 5 graded); a prediction resting on an owner-only fact (the budget) is shown only to the owner view |
...
| `dsr.pipeline` | 2 | the nightly stage machine: `run_night` (never before close; POS closeday poll; block retries 10/20/40 until the local deadline; only Sales is required — provisional only without it, late sales make a new version, any other late block goes out labelled; the closeout never holds; failed after bounded retries; one claim per restaurant/night/version; a re-run is made only from sales that are in; predictions recorded only before the next night opens), `run_sweep` (the scheduler entry, bounded, cursor in `job_cursors`; also runs a night it never created since the restaurant's earliest recent report), `start_manual` (Close day — never overrules a POS with a close-day record; `manual_close_refusal` for one without), `recheck_final` (a final night the POS archive disagrees with, re-pulled; a new version only when the POS's figure moved — `pos._recheck_sales_consistency` calls it). Reaches blocks and the narrative lazily by name |
...
| `dsr.block_labor` | 2 | the Labor block from what the POS sync archived: labor $/%/hours (a mid-service `final=0` row is never the night's; $ and % withheld on Cavnar's assumed $26/hr — `cost_basis`), overtime, vs `notify.labor_target_for` named by `thresholds.target_for`, coverage issues (by business date, late when the punches show them, measured only on a night the check ran), the published day's Shift Quality, deterministic observations |
| `dsr.access` | 2 | who sees which DSR: `view_for(user)` (owner / manager / none), redaction by the existing permission rules (gross goes with the loss lines), narrative filtered by what it cites and by the withheld subjects it names, the per-view tomorrow and yesterday's predictions, the owner's budget read live (`_live_budget`), the stage checklist |
...
| `dsr.rollup` | 1 | a week and a fiscal period from the nightly reports (Erik's weekly grid): categories, gross/net, budget, last year (`store.last_year_day` through `store.baselines_net`, the nightly report's own resolver), labor, the day's notes; totals over measured days only; `access.redact_grid` drops the budget for a manager (and labor / gross without their grants) |
...
| `dsr.block_food` | 2 | Food block: estimated food cost (recipes × the night's item sales — `menu_item_sales`, else tonight's Sales block items matched by the POS item id — labelled an estimate, with coverage), waste logged that night, low / critical stock, recoverable (the deduplicated driver total), variance only where a count exists; `awaiting` only while tonight's Sales block is |
