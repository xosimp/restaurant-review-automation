...
| `dsr.pipeline` | 2 | the nightly stage machine: `run_night` (never before close; POS closeday poll; block retries 10/20/40; provisional at the local deadline; a late block is a new version; failed after bounded retries; one claim per restaurant/night/version), `run_sweep` (the scheduler entry, bounded, cursor in `job_cursors`), `start_manual` (Close day). Reaches blocks and the narrative lazily by name |
