...
| `dsr.kpis` | 2 | the night's key numbers WITH direction for both views (9/25/26): the value, the change against the same weekday (points for a %, percent for money and counts), "Best/Worst <weekday> in N weeks", a 14-night trend line, the target (`thresholds.target_for`) and — only when the Benchmark Engine has a fair one — restaurants like yours; derived sales per labor hour, prime cost % and beverage mix. `OWNER_SET` / `MANAGER_SET` (operations, not finance) and the manager's `shift_recap` and operations, every figure through `dsr.access.line_allowed` |
| `dsr.tomorrow` | 2 | the next night as a report goes out: prep items (approved/pending time off, the NWS forecast, listed events and reservations, critically low stock, the published schedule), Cavnar's forecast and an AI confidence % with what it rests on and what is missing (held at 70% until 10 predictions are graded). Built once by the pipeline (`facts["tomorrow"]`), deterministic |
| `dsr.predictions` | 2 | "How did yesterday turn out?": gradable predictions about the next night (the forecast's range, vs budget, rain, a listed event), written once before the night into `dsr_predictions`, graded from that night's Sales block, and the rolling 30-day accuracy (a % from 5 graded) |
