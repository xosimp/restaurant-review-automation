...
| `rec_learning` | 2 | what the ledger has learned about ONE restaurant: the bounded per-restaurant effectiveness model the rankers read (Home order, the one thing, the DSR's actions — acceptance × success × dollar calibration by kind and tag, shrunk toward the cohort prior over `MIN_COHORT`, a decaying penalty for a worse result, never applied to a critical item), and the owner's own record behind `/recs/summary`, `/recs/timeline` and `/recs/checkin`, redacted per viewer (`viewer_sees`) |
