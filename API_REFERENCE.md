...
- `POST dsr/close` `{date?}` — Close day: starts the night now (trigger `manual`) on a background thread; 202 `{started: true}`, 200 `{started: false, status: "final"}` when it's already final. `date` defaults to tonight's business date; only tonight or the night before (400 otherwise); 409 when `dsr_enabled` is off; rate-limited per restaurant.
