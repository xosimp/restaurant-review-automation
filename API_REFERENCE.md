...
- **`GET outcomes`** `?status=` → `{ok, outcomes: [row], caveat}`. Each row, besides the table's own columns and `metric_label`, `unit`, `summary`, `informational`:
