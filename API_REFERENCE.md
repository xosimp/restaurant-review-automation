...
- `GET dsr/week?date=` / `GET dsr/period?date=` — Erik's weekly grid and the fiscal period (`dsr.rollup` through `access.redact_grid`: budget columns for the owner view only; labor columns only with LABOR_VIEW).
...
- `GET dsr/settings` / `POST dsr/settings` — owner only. GET: `settings` {`fiscal_week_start_dow` (0=Mon…6=Sun), `fiscal_year_start`, `fiscal_period_scheme` (`4x13` | `445`), `dsr_enabled`, `dsr_notify` (email + push the finished report; off by default), `dsr_deadline_hour`, `calendar_label`}, `categories`, `category_map`, and `unmapped` — the departments the latest report couldn't place, with their dollars (`unmapped_as_of`). POST: any of the six settings; Period 1 must start on the week's first day (400 otherwise).
...
- **`GET outcomes`** `?status=&ids=` → `{ok, outcomes: [row], caveat}` — newest 50, or with `ids` (comma-separated, ≤200) exactly those trackers of this restaurant however old (the timeline asks for the ones its page links to; a non-number is 400). Each row also carries `checkin_key` — the recommendation key a check-in for it is sent against (`POST recs/checkin`), `null` when no recommendation stands behind the tracker (a manual one, a monthly reprice figure), so no surface offers a check-in the server would refuse. Each row, besides the table's own columns and `metric_label`, `unit`, `summary`, `informational`:
  ...
  - `concurrent` — `[{kind, label, date}]`, kind one of `tracker` | `accepted_rec` | `price_change` | `event` | `holiday` | `closure`; `concurrent_checked` false for a row evaluated before this existed (its grade is then at most `associated`).
...
- **`GET value`** → `{ok, delivered, avoided, opportunity, surfaced, ledger, promise?, rates}` — the four figures still never summed. `delivered` adds:
  - `worsened` `{count, monthly}` and `net_monthly` (= `monthly` − `worsened.monthly`; may be negative, and `net_note` says so), `net_by_module`. `monthly` stays the improvements alone.
  ...
  - `cumulative` `{total, gained, lost, since, until, days, measured_days, by_module, basis}` — a sum of measured days (`outcome_value_days`), net of changes that got worse; `total` is null when no day has been measured.
...
- **Tracker-start replies.** Every door that can start a tracker answers with exactly one of:
  ...
  Gates: an owner-pressed Track, Ask's `track_outcome`, observed actions, reprice, campaigns and schedule accepts refuse while another tracker measures the **same metric**; `recs/event` Accept/Done and Home Done (automatic starts) refuse while anything in the **same family** is measured. An alert-opened tracker never blocks a real one.
...
| `answerable` | bool | true when the client should offer Done / Not for us (and Track where `strategy_routes.REC_TRACK_METRICS` has the module). False for keys nothing can answer (`money:*`, `urgent_reviews`, `intraday_pulse:*` — never presented), for bookkeeping keys (`standby:`, `calibration:` — their own button is the answer), and for one already answered |
...
- **Morning brief** — each keyed line (`GET morning-brief` → `brief.lines[]`) carries `rec_key` and `answerable` beside the old `rec`. The push payload carries `surface: "brief_push"`, `alert_id`, `rec_key`, `answerable`.
- **Pushes** — every push the server sends with a recommendation carries `rec_key`, `answerable` and `surface` (`alert_push` or `brief_push`); the pre-dinner pulse's `staffing_move` carries `rec_key` / `answerable` (`pulse_cut:<date>:<name>`); the quiet-night push carries `answerable: true`.
...
- **Content calendar** — `GET /api/content-calendar` `ideas[]`, `GET /mobile/api/marketing` `calendar[]`, `POST /mobile/api/marketing/calendar` `calendar[]`: each idea carries `rec_key` (`content_idea:<hash of the angle>`), `answered`, `answerable`; open ideas are presented on `marketing`. Writing from an idea (`from_calendar`) records it `accepted`.
