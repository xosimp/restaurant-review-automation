...
- `GET dsr/week?date=` / `GET dsr/period?date=` — the weekly grid and the period (`dsr.rollup`, redacted by `access.redact_grid`: no budget columns for a manager; `withheld: ["budget"]`). Period answers 409 without a fiscal year start.
...
- `GET dsr/settings` / `POST dsr/settings` — owner only. GET: `settings` {`fiscal_week_start_dow` (0=Mon…6=Sun), `fiscal_year_start`, `fiscal_period_scheme` (`4x13` | `445`), `dsr_enabled`, `dsr_deadline_hour`, `calendar_label`}, `categories`, `category_map`, and `unmapped` — the departments the latest report couldn't place, with their dollars (`unmapped_as_of`). POST: any of the five settings; Period 1 must start on the week's first day (400 otherwise).
- `POST dsr/history/import` (multipart `file`; `{ok, imported, skipped, errors: [...]}`) and `GET dsr/history/template.csv` — last-year import from old DSR workbooks; the web grid's "Import last year" control posts to it (routes owned by the last-year import work).
